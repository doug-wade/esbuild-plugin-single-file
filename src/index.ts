...
const singleFilePlugin = () => ({
    ...
    setup(build: PluginBuild) {
        ...
        const registry = new Map<string, { dom: JSDOM, linkTags: Record<string, ResolveResult>, scriptTags: Record<string, ResolveResult> }>();
        const watchFiles = new Set<string>()

        build.onResolve({ filter: /\.html$/ }, async (args) => {
            const { dom, linkTags, scriptTags } = buildRegistry(args.path);

            const linkTagsWithResolution = Object.fromEntries(await Promise.all(linkTags.map(async (linkTag) => {
            ...
            const scriptTagsWithResolution = Object.fromEntries(await Promise.all(scriptTags.map(async (scriptTag) => {
                const resolveDir = join(args.resolveDir, dirname(args.path));
            ...
            registry.set(args.path, { dom, linkTags: linkTagsWithResolution, scriptTags: scriptTagsWithResolution });
        ...
        build.onLoad({ filter: /.*/, namespace }, async (args) => {
            ...
            await Promise.all(Object.entries(linkTags).map(async ([href, resolved]) => {
                const oldElement = dom.window.document.body.querySelector(`link[href="${href}"]`);
                ...
                if (resolved.path) {
                    ...
                    if (result.outputFiles && result.outputFiles.length > 0) {
                        ...
                        if (result.outputFiles.length > 1 && result.outputFiles[1].path.endsWith('.css')) {
            ...
            await Promise.all(Object.entries(scriptTags).map(async ([src, resolved]) => {
                const oldElement = dom.window.document.querySelector(`script[src="${src}"]`);
